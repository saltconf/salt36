    ...
    {% for event in day.events %}
      {% if event.type == "talk_session" %}
        ...
        <tr class="talkChairinfo">
          <td colspan="3">{{ event.name }} // chair: <a href="{{ chair.website }}" class="chairName">{{ chair.name }}</a></td>
        </tr>
        {% for talk in event.subevents %}
          {% assign talkinfo = site.data.presentations.talks[talk.talkid] %}
          <tr class="talk">
            <td class="time">{{ talk.time }}</td>
            <td class="title">{% include program_title.html info=talkinfo id=talk.talkid %}</td>
      ...
      {% elsif event.type == "poster" %}
        ...
        {% for posterinfo in site.data.presentations.posters %}
          {% if posterinfo[1].poster_session == event.session %}
            <tr class="posterInfo">
              {% assign current_poster_info = posterinfo[1] %}
              {% assign current_poster_id = posterinfo[0] %}
              {% if first_poster %}
              ...
              {% endif %}
              <td class="title">{% include program_title.html info=current_poster_info id=current_poster_id %}</td>
              <td class="authors">{% include program_authors.html author_ids=current_poster_info.authors %}</td>
            </tr>
          {% endif %}
        {% endfor %}
      {% elsif event.type == "keynote" %}
        ...
        <tr class="invitedChairinfo">
          <td colspan="3">Invited Talk // chair: <a href="{{ chair.website }}" class="chairName">{{ chair.name }}</a></td>
        </tr>
        <tr class="invited">
